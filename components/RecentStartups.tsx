  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      const sortedStartups = ids
        .map((id) => fullStartupData.find((startup: StartupTypeCard) => startup._id === id))
  ...
  return (
    <section className="w-full mt-16 pt-8 border-t border-black/5">
      ...
      <div className="relative group mt-4">
        ...
        <ul ref={scrollRef} className="card_grid-sm-editor relative w-full hide-scrollbar">
          {startups.map((post: StartupTypeCard, i: number) => (
            <li key={i} className="snap-center min-w-[260px] md:min-w-[300px]">
              {/* Renders your exact, fully-featured card component */}
